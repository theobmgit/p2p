	...
	try {
		...
		const statusCheckInterval = setInterval(() => {
			if (navigator.serviceWorker.controller && localStorage.getItem("notifyTopic")) {
				window.notificationManager.checkStatus().then(status => {
		...
		const subscriptionHeartbeatInterval = setInterval(() => {
			if (navigator.serviceWorker.controller && localStorage.getItem("notifyTopic")) {
				...
				window.notificationManager.testPushSubscription().then(result => {
					if (!result.success) {
...
async function parseVdoNinjaUrl(url) {
	try {
		...
		if (!url.includes("vdo.ninja") && !url.includes("obs.ninja") && !url.includes("versus.cam") && !url.includes("comms.cam")) {
			throw new Error("Not a valid TL.P2P URL");
		}
		const urlObj = new URL(url);
		...
		console.error("Error parsing TL.P2P URL:", e);
		throw e;
	}
}
...
function setupUrlDetection() {
	...
	if (!urlInput) {
        ...
        <label for="vdo-url-input">TL.P2P URL (Optional):</label>
        <input type="text" id="vdo-url-input" placeholder="Paste your TL.P2P URL">
...
function enhanceTopicInput() {
	...
	if (topicInput) {
		topicInput.addEventListener("paste", async e => {
			...
			if (pastedText && (pastedText.includes("://") || pastedText.includes("vdo.ninja") || pastedText.includes("obs.ninja"))) {
				e.preventDefault();
...
function generateSafeUrl(url) {
	try {
		...
		const urlObj = new URL(url);
		const params = new URLSearchParams(urlObj.search);
		const sensitiveParams = ["password", "pass", "pwd", "hash", "secret", "token", "key", "apikey", "api_key", "auth", "credentials", "cred", "signin", "login"];
	...
	if (fakeNotificationBtn) {
		fakeNotificationBtn.addEventListener("click", () => {
			...
			const testNotification = {
				...
				body: "This is a test notification for TL.P2P",
...
function startNotificationSubscription(topic) {
	if (!topic) return;
	window.notificationManager.startSubscription(topic).then(success => {
		updateSubscriptionStatusPanel();
	});
}
function stopNotificationSubscription() {
	window.notificationManager.stopSubscription().then(() => {
		updateSubscriptionStatusPanel();
	});
}
...
function createNotificationPopup(notification) {
    ...
    <div class="notification-popup-header">
      <h3 class="notification-popup-title">🔔 ${notification.title || "TL.P2P Notification"}</h3>
...
function updateNotificationHistoryUI() {
	...
	notificationHistory.forEach((notification, index) => {
      ...
      <div class="content">
        <h3>${notification.title || "TL.P2P Notification"}</h3>
...
function setupParseUrlButton() {
	...
	parseUrlBtn.addEventListener("click", async () => {
		...
		if (!url) {
			displayStatusMessage("⚠️ Please enter a TL.P2P URL", "warning");
			return;
		}
		try {
