...
class NotificationManager {
	...
	isNotificationDuplicate(notification) {
		...
		return existingNotifications.some(n => {
			return (n.timestamp || n.receivedAt || 0) >= fiveSecondsAgo && n.title === notification.title && n.body === notification.body;
		});
	}
	...
	showNotification(notification) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				self.registration
					.showNotification(notification.title || "TL.P2P", options)
					.then(() => {
	...
	loadNotificationHistory() {
		...
		if (navigator.serviceWorker.controller) {
			this.sendToServiceWorker({ action: "syncNotifications" }).catch(error => console.error("Failed to sync notifications:", error));
		}
	}
