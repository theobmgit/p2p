...
const urlsToCache = ["/notifications/media/tone.mp3", "/notifications/media/chime.wav", "/notifications/media/bell.wav", "/notifications/media/logo2.png", "/notifications/media/icon.png"];
		...
		.then(async () => {
			switch (data.action) {
				...
				case "syncNotifications":
					...
					break;
				case "checkStatus":
					const [topic, sseStatus, pollingStatus] = await Promise.all([getStoreItem("topic"), getStoreItem("sseStatus"), getStoreItem("pollingStatus")]);
					sendResponse({
						action: "statusUpdate",
...
self.addEventListener("periodicsync", event => {
	if (event.tag === "notification-poll") {
		Promise.all([getStoreItem("topic"), getStoreItem("sseStatus")]).then(([topic, sseStatus]) => {
	...
	return setStoreItem("topic", topic).then(() => {
		try {
			...
			sseConnection.onerror = error => {
				...
				if (sseConnection && sseConnection.readyState === 2) {
					// CLOSED
					reconnectSSE();
				}
			};
...
function reconnectSSE() {
	...
	const baseDelay = Math.min(MAX_RECONNECT_DELAY, Math.pow(1.5, Math.min(reconnectAttempts, 8)) * 1000);
...
function showNotification(notification) {
	...
	return Promise.all([testImage("./media/logo2.png"), testImage("./media/icon.png")]).then(([logoExists, iconExists]) => {
		const options = {
		...
		return self.registration.showNotification(notification.title || "TL.P2P", options);
	});
}
...
function disconnectAll() {
	...
	currentTopic = null;
	return Promise.all([setStoreItem("topic", null), setStoreItem("sseStatus", { connected: false }), setStoreItem("pollingStatus", { isPolling: false })]);
}
